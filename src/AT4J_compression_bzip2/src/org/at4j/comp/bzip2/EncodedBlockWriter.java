...
final class EncodedBlockWriter
{
	...
	EncodedBlockWriter(BitOutput out, LogAdapter la)
	{
		m_out = out;
		m_logAdapter = la;
	}
	...
	private void writeBlockInternal(final int blockNo, final EncodedBlockData blockData) throws IOException
	{
		if (blockData == null)
		{
			...
			{
				m_logAdapter.logDebug("Reached the end of the encoded data");
			}
		...
		else
		{
			if (m_logAdapter != null)
			{
				m_logAdapter.logDebug("Writing block #" + blockNo);
			}
			...
			while (m_savedBlocks.containsKey(++m_nextBlockToWrite))
			{
				...
				if (savedBd != null)
				{
					if (m_logAdapter != null)
					{
						m_logAdapter.logDebug("Writing (stored) block #" + m_nextBlockToWrite);
					}
				...
				else
				{
					if (m_logAdapter != null)
					{
						m_logAdapter.logDebug("Reached the end of the encoded data (stored).");
					}
					m_doneLatch.countDown();
					break;
				}
