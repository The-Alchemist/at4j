...
public final class UnsignedByte implements Serializable, Comparable<UnsignedByte>
{
	...
	/**
	 * Read an unsigned byte value from the supplied {@link RandomAccess}. The
	 * file pointer is advanced one position by this method.
	 * @param ra The {@code RandomAccess} object to read from.
	 * @return An unsigned byte read from the file.
	 * @throws UnexpectedEofException If the file pointer is at the end of the
	 * file.
	 * @throws WrappedIOException On I/O errors.
	 */
	public static UnsignedByte read(RandomAccess ra) throws UnexpectedEofException, WrappedIOException
	{
		int read = ra.read();
		if (read < 0)
		{
		...
		return valueOfSafe(read);
	}

	/**
	 * Read an unsigned byte value from the supplied {@link InputStream}.
	 * @param is The stream to read from.
	 * @return An unsigned byte read from the stream.
	 * @throws UnexpectedEofException If no more data could be read from the
	 * stream.
	 * @throws WrappedIOException On I/O errors.
	 */
	public static UnsignedByte read(InputStream is) throws UnexpectedEofException, WrappedIOException
	{
		try
		{
			int read = is.read();
