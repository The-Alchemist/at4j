...
public final class UnsignedShort implements Serializable, Comparable<UnsignedShort>
{
	 ...
	 * current position in the supplied {@link RandomAccess}. The most
	 * significant byte is the last byte read.
	 * @param ra The {@code RandomAccess} to read from. The current position is
	 ...
	 */
	public static UnsignedShort readBigEndian(RandomAccess ra) throws UnexpectedEofException, WrappedIOException
	{
		byte[] barr = new byte[2];
		int noRead = ra.read(barr);
		if (noRead != 2)
		{
			throw new UnexpectedEofException("Wanted to read 2 bytes. Got " + noRead);
		}
		return fromBigEndianByteArray(barr);
	}

	/**
	 * Create an unsigned short value from two bytes read from the stream. The
	 * most significant byte is the last byte read.
	 * @param is The stream to read from.
	 * @return An unsigned short.
	 * @throws UnexpectedEofException If the end of the stream is reached before
	 * two bytes have been read.
	 * @throws WrappedIOException On I/O errors.
	 */
	public static UnsignedShort readBigEndian(InputStream is) throws UnexpectedEofException, WrappedIOException
	{
		byte[] barr = new byte[2];
		try
		{
