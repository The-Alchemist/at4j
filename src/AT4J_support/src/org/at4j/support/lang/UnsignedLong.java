...
public final class UnsignedLong implements Serializable, Comparable<UnsignedLong>
{
	 ...
	 */
	public static UnsignedLong readBigEndian(RandomAccess ra) throws UnexpectedEofException, WrappedIOException
	{
		byte[] barr = new byte[8];
		int noRead = ra.read(barr);
		if (noRead != 8)
		{
			throw new UnexpectedEofException("Wanted to read 8 bytes. Got " + noRead);
		}
		return fromBigEndianByteArray(barr);
	}

	/**
	 * Create an unsigned long value from eight bytes read from the stream. The
	 ...
	 * @throws WrappedIOException On I/O errors.
	 */
	public static UnsignedLong readBigEndian(InputStream is) throws UnexpectedEofException, WrappedIOException
	{
		byte[] barr = new byte[8];
		try
		{
